...
class MusicBot(discord.Client):
    ...
    async def on_player_finished_playing(self, player, **_):
        await self.unskip(unskip_state=False)
    ...
    async def on_ready(self):
        ...
        if self.config.bound_channels:
            ...
            invalids.update(c for c in chlist if c.type != discord.ChannelType.text) #invalids.update(c for c in chlist if c.type == discord.ChannelType.voice)
            ...
            if invalids and self.config.debug_mode:
                print("\nCannot binding to non-text channels:")
        ...
        if self.config.autojoin_channels:
            ...
            # If you setup wrong id in options.ini, you will be stucked here.
            invalids.update(c for c in chlist if c.type != discord.ChannelType.voice) #invalids.update(c for c in chlist if c.type == discord.ChannelType.text)
            chlist.difference_update(invalids)
            ...
            if invalids and self.config.debug_mode:
                print("\nCannot join non-voice channels:")
                [self.safe_print(' - %s/%s' % (ch.server.name.strip(), ch.name.strip())) for ch in invalids if ch]
    ...
    async def cmd_h(self, command=None):
        """
        Usage:
            {command_prefix}h
        ...
        return await self.cmd_help(command=command)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.url: # song_url does not exist
                ...
                url = '?' # player.current_entry.song_url #str(player.playlist.entries[0].url)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if (author.id == self.config.owner_id and not self.unskip_state) or (permissions.instaskip and not self.unskip_state) or (author == player.current_entry.meta.get('author', None) and not self.unskip_state):
            ...
            def check(m):
                return (
                    m.content.lower()[0] in 'yn' or
                    m.content.lower().startswith('{}{}'.format(self.config.command_prefix, 'skip')) or
                    m.content.lower().startswith('{}{}'.format(self.config.command_prefix, 'd')))

            confirm_message = await self.safe_send_message(channel, "Player staying in __**unskip state**__. But you can skip it. Type `y` if you want to skip or `n` if you want to continue playing.")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(confirm_message)
                return Response("Ok nevermind.", delete_after=20)

            if response_message.content.lower().startswith('y'):
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)
                player.skip()
                return Response("Alright, listen next track!", delete_after=10)
            else:
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)
            return Response("Ok, continue listening.", delete_after=10)

        elif author.id != self.config.owner_id and self.unskip_state:
            return Response("Player staying in __**unskip state**__, only owner can skip this track.", delete_after=20)
    ...
    async def cmd_d(self, player, channel, author, message, permissions, voice_channel):
        """
        Usage:
            {command_prefix}d
        ...
        return await self.cmd_skip(player, channel, author, message, permissions, voice_channel) 
    ...
    async def cmd_undo(self, author, player, channel):
        """
        Usage:
		    {command_prefix}undo
    ...
    async def cmd_u(self, author, player, channel):
        """
        Usage:
            {command_prefix}u
        ...
        return await self.cmd_undo(author, channel, player)
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        Changes the bot's username or use `Default` to back original name.
        Note: This operation is limited by discord to twice per hour.
        """

        name = ' '.join([name, *leftover_args])

        if not name or name == 'Default' or name == 'None':
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        Changes the bot's nickname or use `None` or `Default` to remove bot nick.
        """
        ...
        if not nick or nick == 'Default' or nick == 'None':
            nick = None
        try:
    ...
    async def cmd_unskip(self, author, player, channel, permissions):
        """
        Usage:
        ...
        return Response("This song **%s** now can't be skipped until it end." % player.current_entry.title, delete_after=20)

    async def unskip(self, unskip_state=False):
        """
        Lock skip commands
        """
    ...
    async def cmd_zalgo(self, channel):
        """
        Usage:
            {command_prefix}zalgo
        zalgo text which messes up chat
        """
        await self.safe_send_message(channel, "ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ﻿ฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํํํํํํํํํํํํํํํฏ๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎๎ํํํ")
	
    async def cmd_ffmusic():
        """
        Usage:
        Plays Final Fantasy tunes. Curated by Gyproc
        """

        return await self.cmd_play(https://tinyurl.com/dffoomusic)
